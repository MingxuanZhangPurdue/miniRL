...
Reading order, and what each note underpins in the repo:
...
1. **[pg_to_ppo_derivation.md](pg_to_ppo_derivation.md)** — policy gradient →
   ...
   under every surrogate in `algos/` (and the reason the repo can skip PPO
   itself: see DESIGN non-goals).
2. **[kl_estimators.md](kl_estimators.md)** — k1/k2/k3 KL estimators + the
   k3 unbiasedness proof. Underpins the KL penalty in `algos/grpo.py` and the
   `approx_kl` metric in every loss.
3. **[ppo_to_cispo_derivation.md](ppo_to_cispo_derivation.md)** — why CISPO
   needs an explicit `log π` term (what stop-gradient does to the derivative).
   Underpins `algos/cispo.py`.
4. **[dpo_derivation.md](dpo_derivation.md)** — the DPO objective from the
